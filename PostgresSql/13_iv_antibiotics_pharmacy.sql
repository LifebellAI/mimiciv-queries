...
$do$
BEGIN
   IF  EXISTS (SELECT FROM public.iv_antibiotics_pharmacy) THEN
        DROP TABLE public.iv_antibiotics_pharmacy;
    END IF;
END
$do$;

CREATE TABLE public.iv_abx_pharmacy AS

SELECT
subject_id,
hadm_id,
stay_id,
DATETIME_TRUNC('hour', starttime) AS chart_hour,
medication,
...
AND
DATETIME_TRUNC('hour', starttime) < outtime
AND
DATETIME_TRUNC('hour', starttime) > intime
